...
GenerateInternalTypes(
    Path.Combine(serviceModelsDir, "eventbridge-2015-10-07.normal.json"),
    "EventBridge",
    "LocalSqsSnsMessaging.EventBridge.Model",
    modelsDir);
...
// Generate EventBridge handler (JSON protocol, json 1.1)
GenerateJsonProtocolHandler(
    Path.Combine(serviceModelsDir, "eventbridge-2015-10-07.normal.json"),
    "EventBridge",
    "LocalSqsSnsMessaging.EventBridge.Model",
...
static void GenerateJsonProtocolHandler(string modelPath, string serviceName, string modelNamespace, string outputDir)
{
    ...
    var jsonVersion = metadata.TryGetProperty("jsonVersion", out var jv) ? jv.GetString() ?? "1.0" : "1.0";
    var contentType = $"application/x-amz-json-{jsonVersion}";
    var errorPrefix = $"com.amazonaws.{metadata.GetProperty("endpointPrefix").GetString()}";

    // Generate the handler (library path, HttpRequestMessage - wrapped in #if !ASPNETCORE)
    GenerateJsonHandler(serviceName, modelNamespace, operations, outputDir, contentType, errorPrefix);

    // Generate the HttpContext-native handler (used by both library and server)
    GenerateJsonHttpContextHandler(serviceName, modelNamespace, operations, outputDir, contentType, errorPrefix);
...
static void GenerateJsonHandler(string serviceName, string modelNamespace, JsonElement operations, string outputDir, string contentType, string errorPrefix)
{
    ...
    foreach (var operation in operations.EnumerateObject())
    {
        ...
        code.AppendLine($"            {serviceName}JsonSerializers.Serialize{opName}Response(result, pooledBuffer.Writer);");
        code.AppendLine();
        code.AppendLine("            var response = new HttpResponseMessage(HttpStatusCode.OK)");
        code.AppendLine("            {");
        code.AppendLine("                Content = new ByteArrayContent(pooledBuffer.Writer.WrittenSpan.ToArray())");
        code.AppendLine("            };");
        code.AppendLine($"            response.Content.Headers.ContentType = new System.Net.Http.Headers.MediaTypeHeaderValue(\"{contentType}\");");
    ...
    code.AppendLine("        }");
    code.AppendLine();
    code.AppendLine($"        var errorType = $\"{errorPrefix}#{{errorCode}}\";");
    code.AppendLine();
    ...
    code.AppendLine("        {");
    code.AppendLine($"            Content = new StringContent(errorJson, Encoding.UTF8, \"{contentType}\")");
    code.AppendLine("        };");
    code.AppendLine("    }");
...
static void GenerateJsonHttpContextHandler(string serviceName, string modelNamespace, JsonElement operations, string outputDir, string contentType, string errorPrefix)
{
    var code = new StringBuilder();
    code.AppendLine("// <auto-generated/>");
    ...
    foreach (var operation in operations.EnumerateObject())
    {
        ...
        code.AppendLine($"            context.Response.ContentType = \"{contentType}\";");
        code.AppendLine("            using var pooledBuffer = PooledArrayBufferWriter.Rent();");
        code.AppendLine($"            {serviceName}JsonSerializers.Serialize{opName}Response(result, pooledBuffer.Writer);");
...
static void GenerateQueryHandler(string serviceName, string modelNamespace, JsonElement operations, string outputDir)
{
    ...
    foreach (var operation in operations.EnumerateObject())
    {
        ...
        code.AppendLine("                Content = new ByteArrayContent(pooledBuffer.Writer.WrittenSpan.ToArray())");
        code.AppendLine("            };");
        code.AppendLine("            response.Content.Headers.ContentType = new MediaTypeHeaderValue(\"text/xml\") { CharSet = \"utf-8\" };");
        code.AppendLine("            return response;");
        code.AppendLine("        }");
        code.AppendLine("        catch (OperationCanceledException)");
        code.AppendLine("        {");
