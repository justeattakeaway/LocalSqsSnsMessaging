...
/// <summary>
/// Content-based JSON pattern matching shared by EventBridge event patterns and SNS filter
/// policies. Both services use the same grammar: a pattern is a JSON object whose leaf values
/// are arrays of candidates (literals or operator objects such as <c>prefix</c>,
/// <c>anything-but</c>, <c>numeric</c>, <c>exists</c>), and a document matches when every
/// key in the pattern matches at least one candidate.
/// See https://docs.aws.amazon.com/eventbridge/latest/userguide/eb-event-patterns.html and
/// https://docs.aws.amazon.com/sns/latest/dg/sns-subscription-filter-policies.html
/// </summary>
internal static class JsonPatternMatcher
{
    public static bool Matches(JsonObject pattern, JsonObject? document)
    {
        foreach (var (key, value) in pattern)
        {
            if (string.Equals(key, "$or", StringComparison.Ordinal))
            {
                if (value is not JsonArray orArr || !orArr.Any(sub => sub is JsonObject o && Matches(o, document)))
                {
                    return false;
                }
                continue;
            ...
            var present = document is not null && document.TryGetPropertyValue(key, out _);
            JsonNode? fieldValue = null;
            document?.TryGetPropertyValue(key, out fieldValue);
    ...
    private static bool MatchNested(JsonObject nested, JsonNode? fieldValue)
    {
        ...
        {
            JsonObject o => Matches(nested, o),
            JsonArray arr => arr.Any(el => el is JsonObject eo && Matches(nested, eo)),
            _ => false
        };
    }
